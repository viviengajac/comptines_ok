...
{
//	m_onglets: Bloc[];
	override m_classe_boutons: string[];
	m_nb_lignes:number;
	gridApi: any;
	gridColumnApi: any;
	m_grid_api_initialisee:boolean=false;
	m_grid_options:OptionsGrille;
...
//console.log('EcranGrille.AfficherBloc: num_bloc_actif='+this.m_num_bloc_actif);
		var string_json:string=this.m_blocs[this.m_num_bloc_actif].AfficherBloc(RemplacerNomColParHeader,PourExcel);
...
console.log('EEE Bloc.AfficherBloc: string_json='+string_json);
...
console.log('onexcelref: string_json='+string_json);
		var contenu_blocs: string[] = [JSON.parse(string_json)];
		console.log('onexcelref: contenu_blocs='+contenu_blocs);
	...
	onCellValueChanged(event)
	{
		try
		{
			...
			if(event.newValue===undefined)
				val_col_new="";
			var nom_col_modifiee:string=event['column']['colId'];
			...
			var id_cle_primaire:number=selectedRow[nom_col_cle_primaire];
...
//console.log('classe de la coldef='+this.m_grille.m_coldefs[num_col].constructor.name);
